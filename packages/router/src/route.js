...
export function Route ({path, name, defaults}) {
  const keys = []
  ...
  function parse (pathname) {
    ...
    const pathVariables = defaults == null ? {} : clone(defaults)
  ...
  function match (pathname) {
    ...
    const isExactMatch = pathnameParts.length === pathParts.length ||
      (isFinalParameterOptional && pathnameParts.length === pathParts.length - 1)
