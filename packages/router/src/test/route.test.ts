...
test('route parsing', () => {
  ...
  expect(new Route({path: '/docs/:id/foo/:foo', name: ''}).parse('/docs/5')).toEqual(null)

  expect(new Route({path: '/encoding/:test', name: ''}).parse('/encoding/test%2F%20%5E%20%C2%B4%20ersdf')).toEqual({test: 'test/ ^ ´ ersdf'})
})

test('route compilation', () => {
  expect(new Route({path: '/docs/:id/foo/:foo', name: ''}).compile({id: 100, foo: 'test'})).toEqual('/docs/100/foo/test')
  expect(new Route({path: '/encoding/:test', name: ''}).compile({test: 'test/ ^ ´ ersdf'})).toEqual('/encoding/test%2F%20%5E%20%C2%B4%20ersdf')
})
